"""A-B-C-D end-to-end: g2b_api + alio + g2b_crawler(deprecated) → DB → filter →
notifier payloads → dashboard helpers. All network mocked.
"""
import json
from pathlib import Path

from collectors import g2b_api, alio_crawler, g2b_crawler
...
def test_main_run_collect_skips_sources_with_missing_keys(tmp_path, monkeypatch, caplog):
    ...
    cfg = {
        "collection": {
            "sources": {"g2b_api": True, "alio": True, "g2b_crawler": True},
...
def test_main_run_collect_invokes_active_collectors(tmp_path, monkeypatch):
    cfg = {
        "collection": {
            "sources": {"g2b_api": True, "alio": True, "g2b_crawler": False},
            "request_sleep_seconds": 0,
            "page_size": 100,
            "lookback_days": 1,
    ...
    total = main.run_collect(cfg)
    assert total == 2
    assert calls == {"g2b": 1, "alio": 1}

    counts = database.count_by_source(cfg["database"]["path"])
    assert counts == {"g2b_api_thng": 1, "alio": 1}
